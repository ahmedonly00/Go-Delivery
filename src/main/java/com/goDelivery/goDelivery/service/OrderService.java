...
public class OrderService {
    ...
    private OrderResponse createSingleRestaurantOrder(
        ...
        Restaurant restaurant = restaurantRepository.findByRestaurantId(restaurantOrder.getRestaurantId())
                .orElseThrow(() -> new ResourceNotFoundException(
                        "Restaurant not found with id: " + restaurantOrder.getRestaurantId()));
        ...
        order.setOrderStatus(
                orderRequest.getOrderStatus() != null ? orderRequest.getOrderStatus() : OrderStatus.PLACED);
        order.setPaymentStatus(
                orderRequest.getPaymentStatus() != null ? orderRequest.getPaymentStatus() : PaymentStatus.PENDING);
        ...
        float finalAmount = (float) totalAmount + deliveryFee
                - (orderRequest.getDiscountAmount() != null ? orderRequest.getDiscountAmount() : 0.0f);
    ...
    public OrderResponse cancelOrder(Long orderId, String cancellationReason) {
        ...
        if (order.getOrderStatus() == OrderStatus.DELIVERED || order.getOrderStatus() == OrderStatus.CANCELLED) {
            throw new IllegalStateException(
                    "Cannot cancel an order that is already completed " + order.getOrderStatus());
        }
    ...
    public OrderTrackingResponse getOrderTrackingInfo(Long orderId) {
        ...
        if (order.getBikers() != null) {
            ...
            if (biker != null) {
                ...
                deliveryPersonRating = biker.getRating() != null ? biker.getRating().doubleValue() : 4.5; // Default to
                                                                                                          // 4.5 if null
            }
        }
    ...
    private List<OrderTrackingResponse.StatusUpdate> createStatusHistory(Order order) {
        ...
        if (order.getOrderStatus().ordinal() >= OrderStatus.CONFIRMED.ordinal()) {
            history.add(createStatusUpdate(OrderStatus.CONFIRMED, "Restaurant has confirmed your order",
                    order.getOrderConfirmedAt()));
        }

        if (order.getOrderStatus().ordinal() >= OrderStatus.PREPARING.ordinal()) {
            history.add(createStatusUpdate(OrderStatus.PREPARING, "Restaurant is preparing your food",
                    order.getOrderPreparedAt()));
        }

        if (order.getOrderStatus().ordinal() >= OrderStatus.READY.ordinal()) {
            history.add(createStatusUpdate(OrderStatus.READY, "Your order is ready for pickup",
                    order.getOrderPreparedAt()));
        }

        if (order.getOrderStatus().ordinal() >= OrderStatus.PICKED_UP.ordinal()) {
            history.add(createStatusUpdate(OrderStatus.PICKED_UP, "Delivery person has picked up your order",
                    order.getPickedUpAt()));
        }
        ...
        if (order.getOrderStatus() == OrderStatus.DELIVERED) {
            ...
            history.add(createStatusUpdate(OrderStatus.CANCELLED,
                    "Order was cancelled: " + order.getCancellationReason(), order.getCancelledAt()));
        }
    ...
    private OrderTrackingResponse.StatusUpdate createStatusUpdate(OrderStatus status, String message,
            LocalDate timestamp) {
    ...
    public RestaurantRevenueDTO getRestaurantRevenue(Long restaurantId) {
        ...
        double thisWeekRevenue = validOrders.stream()
                .filter(order -> {
                    LocalDate orderDate = order.getDeliveredAt() != null ? order.getDeliveredAt()
                            : order.getOrderConfirmedAt();
        ...
        Map<LocalDate, Double> dailyRevenue = validOrders.stream()
                ...
                .collect(Collectors.groupingBy(
                        order -> order.getDeliveredAt() != null ? order.getDeliveredAt() : order.getOrderConfirmedAt(),
                        TreeMap::new,
    ...
    // private double applyRestaurantDiscounts(Restaurant restaurant, double amount,
    // Long promotionId) {
    // double discount = 0.0;
    ...
    // return discount;
    // }
    // private double calculateDeliveryFee(Restaurant restaurant, String
    // deliveryAddress, double orderAmount) {
    // // Base delivery fee
    ...
    // double distance = calculateDistance(restaurant.getAddress(),
    // deliveryAddress);
    // if (distance > 5) { // 5km
    ...
    // }
    // private double applyPlatformWideDiscounts(double amount, String
    // promotionCode) {
    ...
    // return Math.min(discount, amount * 0.3); // Max 30% discount from customer
    // benefits
    // }
}
