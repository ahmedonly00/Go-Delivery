...
public class PaymentService {
            ...
            switch (paymentRequest.getPaymentMethod()) {
                ...
                default:
                    throw new IllegalArgumentException(
                            "Unsupported payment method: " + paymentRequest.getPaymentMethod());
            }
        } catch (Exception e) {
        ...
        try {
            ...
            if (mpesaResponse != null && mpesaResponse.getTransactionId() != null) {
                ...
                payment.setGateWayResponse(
                        "MPESA payment initiated. Transaction ID: " + mpesaResponse.getTransactionId());
                ...
                String errorMsg = mpesaResponse != null ? "Failed to initiate MPESA payment. Status: " +
                        (mpesaResponse.getTransactionStatus() != null ? mpesaResponse.getTransactionStatus()
                                : "Unknown error")
    ...
    public void handleMpesaWebhook(MpesaWebhookRequest webhookRequest) {
        ...
        if (attributes != null && attributes.getRequest() != null) {
            ...
            // Log all headers for debugging (sensitive info should be redacted in
            // production)
        ...
        // Validate signature if not in development mode
            ...
            log.warn(
                    "Invalid or missing webhook signature. Possible tampering detected or not called in an HTTP request context.");
            ...
            try {
                ...
                switch (webhookRequest.getTransactionStatus().toUpperCase()) {
                    ...
                    default:
                        ...
                        payment.setGateWayResponse("Payment status: " + webhookRequest.getTransactionStatus() + ". "
                                + webhookRequest.getDescription());
                }
    ...
    private boolean isValidSignature(MpesaWebhookRequest webhookRequest, String receivedSignature) {
        ...
        try {
            ...
            if (apiKey == null || apiKey.trim().isEmpty()) {
                log.error(
                        "MPESA API key is not configured. Please set 'mpesa.api-key' in your application properties.");
                return false;
            }

            // Log the input values for debugging (be careful with sensitive data in
            // production)
            ...
            // Note: The exact format should match what MPESA is using to generate the
            // signature
    ...
    private PaymentResponse processMomoPayment(Payment payment) {
        ...
        try {
            ...
            if (momoResponse != null && momoResponse.getReferenceId() != null) {
                ...
                String errorMsg = momoResponse != null ? "Failed to initiate MoMo payment. " +
            ...
            if (e.getMessage() != null && e.getMessage().contains("temporarily unavailable")) {
                ...
                payment.setPaymentStatus(PaymentStatus.PENDING);
                payment.setGateWayResponse(
                        "MoMo service temporarily unavailable. Please try again later or use cash payment.");
                payment = paymentRepository.save(payment);

                // Return response with alternative options
                PaymentResponse response = paymentMapper.toDto(payment);
                response.setMessage(
                        "MoMo service is temporarily unavailable. Please try again later or choose cash payment on delivery.");
