...
import com.goDelivery.goDelivery.shared.config.OrderConfig;
...
import com.goDelivery.goDelivery.modules.payment.repository.MomoTransactionRepository;
...
public class MomoService {
    ...
    public CollectionDisbursementResponse initiateCollectionDisbursement(CollectionDisbursementRequest request) {
        try {
            ...
            request.getDisbursementRecipients()
                    .forEach(recipient -> recipient.setMsisdn(formatMsisdn(recipient.getMsisdn())));
            ...
            log.info("Number of disbursement recipients: {}", request.getDisbursementRecipients().size());

            // Log each recipient
            for (int i = 0; i < request.getDisbursementRecipients().size(); i++) {
                var recipient = request.getDisbursementRecipients().get(i);
                log.info("Recipient {}: externalId={}, msisdn={}, amount={}",
                        i + 1, recipient.getExternalId(), recipient.getMsisdn(), recipient.getAmount());
            }
            ...
            if (response.getStatusCode() == HttpStatus.OK && response.getBody() != null) {
                log.info("Collection-disbursement initiated successfully. Reference ID: {}",
                ...
                String errorMsg = String.format("Failed to initiate collection-disbursement: %d - %s",
            ...
            log.error("MoMo API server error ({}): {}", e.getStatusCode(), e.getResponseBodyAsString());
            log.error("Request that caused the error - Collection External ID: {}, Amount: {}, Recipients: {}",
                    request.getCollectionExternalId(),
                    request.getCollectionAmount(),
                    request.getDisbursementRecipients().size());
            throw new RuntimeException("MoMo API server error: " + e.getResponseBodyAsString(), e);
        } catch (org.springframework.web.client.HttpClientErrorException e) {
            // Log detailed error information for 4xx errors
            log.error("MoMo API client error ({}): {}", e.getStatusCode(), e.getResponseBodyAsString());
            throw new RuntimeException("MoMo API client error: " + e.getResponseBodyAsString(), e);
        } catch (Exception e) {
            log.error("Error processing collection-disbursement", e);
            throw new RuntimeException("Failed to process collection-disbursement: " + e.getMessage(), e);
        }
    }
        ...
        try {
            ...
            if (e.getCause() instanceof java.net.ConnectException ||
                ...
                String errorMsg = String.format("""
                        ...
                        authUrl);
                log.error(errorMsg);
                throw new RuntimeException("Authentication request failed: Payment service is temporarily unavailable",
                        e);
            } else {
    ...
    private void sendTransactionNotifications(MomoTransaction transaction) {
        ...
        switch (transaction.getStatus()) {
            ...
            case CANCELLED:
                ...
                cancelledData.put("message",
                        "Your payment has been cancelled. If this was unexpected, please contact support.");
