...
public class MpesaPaymentService {
    ...
    @Retryable(value = {
            WebClientResponseException.class }, maxAttemptsExpression = "${mpesa.max-retries:3}", backoff = @Backoff(delayExpression = "${mpesa.retry-delay:1000}", multiplier = 2))
    @Transactional
    public Mono<MpesaPaymentResponse> initiatePayment(MpesaPaymentRequest request) {
        ...
        return client.get()
                ...
                .flatMap(response -> {
                    try {
                        ...
                        if (rootNode.has("data") && rootNode.get("data").isArray() && rootNode.get("data").size() > 0) {
                            ...
                            if (paymentResponse.getThirdPartyRef() == null
                                    || paymentResponse.getThirdPartyRef().isEmpty()) {
                                String thirdPartyRef = "REF_"
                        ...
                        // If we have an error message
                            ...
                            String errorMsg = rootNode.has("message") ? rootNode.get("message").asText()
                                    : "Unknown error from MPESA API";
                        ...
                        return Mono
                                .error(new RuntimeException("Failed to parse MPESA API response: " + e.getMessage()));
                    }
                })
                .onErrorResume(e -> {
                ...
                .doOnSuccess(response -> log.info("MPESA payment initiated. Transaction ID: {}",
                        response.getTransactionId() != null ? response.getTransactionId() : "N/A"))
                .doOnError(error -> log.error("Error initiating MPESA payment: {}", error.getMessage(), error));
    }
    ...
    public void processWebhook(String payload, String signature) {
        try {
            ...
            // If the transaction is completed and has an associated order, we can clean up
            // retry tracking
        ...
        if (webhookData.has("thirdPartyRef")) {
            ...
            if (transaction.getOrder() == null && StringUtils.hasText(thirdPartyRef)) {
                try {
                    ...
                    if (order != null) {
                        transaction.setOrder(order);
                        log.info("Linked transaction {} to order {}", transaction.getTransactionId(),
                                order.getOrderId());
                    }
                } catch (Exception e) {
    ...
    @Retryable(value = {
            WebClientResponseException.class }, maxAttemptsExpression = "${mpesa.max-retries:3}", backoff = @Backoff(delayExpression = "${mpesa.retry-delay:1000}"))
    public Mono<MpesaPaymentResponse> queryTransactionStatus(String transactionId) {
        ...
        return webClient.get()
                ...
                .doOnSuccess(response -> log.info("Retrieved transaction status for {}: {}",
                        transactionId, response.getTransactionStatus()))
                .doOnError(error -> log.error("Error querying transaction status: {}", error.getMessage()));
    }
            ...
            try {
                notificationService.sendEmail(
                        ...
                        Map.of(
                                ...
                                "failureReason",
                                webhookRequest.getDescription() != null ? webhookRequest.getDescription()
    ...
    protected void schedulePaymentStatusCheck(Long orderId, String transactionId, int delayInMinutes) {
        try {
            ...
            long delay = (long) (delayInMinutes * Math.pow(backoffMultiplier, currentRetry)) * 60 * 1000; // Convert to
                                                                                                          // milliseconds
        ...
        try {
            ...
            return webClient.get()
                    ...
                    .onStatus(status -> status.is4xxClientError() || status.is5xxServerError(), response -> {
                        ...
                        return Mono.error(
                                new RuntimeException("Failed to query transaction status: " + response.statusCode()));
                    })
