...
public class BranchSetupService {
    ...
    // Comprehensive branch setup - single API for branch manager to complete all
    // remaining setup.
    @Transactional
    public BranchesDTO completeBranchManagerSetup(Long branchId, BranchManagerSetupDTO setupDTO) {
        ...
        // Handle file uploads
        try {
            if (setupDTO.getCommercialRegistrationFile() != null
                    && !setupDTO.getCommercialRegistrationFile().isEmpty()) {
    ...
    private void updateOrCreateOperatingHours(Branches branch, BranchManagerSetupDTO dto) {
        ...
        // Update hours from nested DTO (same for all days as per simple
        // OperatingHoursDTO)
